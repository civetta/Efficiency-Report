def split_sheet_by_days(wb, skip_days):
    ...
    while True:
        ...
        if any(skip in current_time_cell for skip in skip_days):
            current_row = find_next_day(raw_change_ws, current_row, max_row)
        current_row = find_start(raw_change_ws, current_row, max_column, max_row)
        end_of_day_row = find_end(raw_change_ws, current_row, max_column, max_row)
