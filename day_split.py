...
def make_sheets(wb,current_row,end_of_day_row,raw_changes_ws,max_column,day):
    ...
    for column in range(1,max_column):
        val=raw_changes_ws.cell(row=1,column=column).value
        current_sheet.cell(row=1,column=column,value=val)
        rower=2
        for row in range(current_row,end_of_day_row):
            value=raw_changes_ws.cell(row=row,column=column).value
            current_sheet.cell(row=rower,column=column,value=value)
            rower=rower+1
